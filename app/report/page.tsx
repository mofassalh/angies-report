      ...
      <div style={{width:220,flexShrink:0,height:'100vh',background:'#fff',borderRight:'1px solid #e5e5e5',display:'flex',flexDirection:'column'}}>
        ...
        <nav style={{flex:1,padding:12,display:'flex',flexDirection:'column',gap:4}}>
          {[
            ...
            { label:'Reporting', icon:FileText, children:[
            ...
            <div key={section.label} style={{marginBottom:4}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:1}}>
                {section.children.map((child: {label:string,href:string}) => (
                  <Link key={child.href} href={child.href} style={{display:'block',padding:'7px 10px 7px 28px',borderRadius:8,textDecoration:'none',fontSize:13,background:child.href==='/report'?'#F5C800':'transparent',color:child.href==='/report'?'#1A1A1A':'#555',fontWeight:child.href==='/report'?600:400}}>
                    {child.label}
                  </Link>
                ))}
              </div>
